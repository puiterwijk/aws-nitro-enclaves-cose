...
impl COSEAlgorithm {
    ...
    fn tag_size(&self) -> usize {
        match self {
            COSEAlgorithm::A128GCM => 16,
            COSEAlgorithm::A192GCM => 16,
            COSEAlgorithm::A256GCM => 16,
            COSEAlgorithm::AesCcm16_64_128 => 8,
            COSEAlgorithm::AesCcm16_64_256 => 8,
            COSEAlgorithm::AesCcm64_64_128 => 8,
            COSEAlgorithm::AesCcm64_64_256 => 8,
            COSEAlgorithm::AesCcm16_128_128 => 16,
            COSEAlgorithm::AesCcm16_128_256 => 16,
            COSEAlgorithm::AesCcm64_128_128 => 16,
            COSEAlgorithm::AesCcm64_128_256 => 16,
        }
    }

    fn openssl_cipher(value: i8) -> Option<Cipher> {
        Some(match value {
            1 => Cipher::aes_128_gcm(),
            2 => Cipher::aes_192_gcm(),
            3 => Cipher::aes_256_gcm(),
            30 => Cipher::aes_128_ccm(),
            31 => Cipher::aes_256_ccm(),
            _ => return None,
        })
    }

    fn from_nid(nid: openssl::nid::Nid) -> Option<COSEAlgorithm> {
        Some(match nid {
            openssl::nid::Nid::AES_128_GCM => COSEAlgorithm::A128GCM,
            openssl::nid::Nid::AES_192_GCM => COSEAlgorithm::A192GCM,
            openssl::nid::Nid::AES_256_GCM => COSEAlgorithm::A256GCM,
            openssl::nid::Nid::AES_128_CCM => COSEAlgorithm::AesCcm16_128_128,
            openssl::nid::Nid::AES_256_CCM => COSEAlgorithm::AesCcm16_128_256,
            _ => return None,
        })
    }
}
...
impl COSEEncrypt0 {
    ...
    pub fn new(payload: &[u8], cipher: Cipher, key: &[u8]) -> Result<Self, COSEError> {
        let mut iv = vec![0; cipher.iv_len().unwrap()];
        rand_bytes(&mut iv).unwrap();

        let cose_alg = match COSEAlgorithm::from_nid(cipher.nid()) {
            None => {
        ...
        protected.insert(1.into(), CborValue::Integer(cose_alg_value as i128));
        let mut unprotected = HeaderMap::new();
        unprotected.insert(5.into(), CborValue::Bytes(iv.to_owned()));
    ...
    pub fn decrypt(&self, key: &[u8]) -> Result<(HeaderMap, &HeaderMap, Vec<u8>), COSEError> {
        ...
        let cipher = match COSEAlgorithm::openssl_cipher(*protected_enc_alg as i8) {
            None => {
                return Err(COSEError::UnsupportedError(
                    "Unsupported openssl encryption cipher".to_string(),
                ))
            }
            Some(v) => v,
        };
...
mod tests {
    ...
    fn test_encrypt_decrypt() {
        ...
        let cencrypt0 = COSEEncrypt0::new(plaintext, Cipher::aes_128_gcm(), key).unwrap();
    ...
    fn test_encrypt_unsupported_alg() {
        let key = b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F";
        let plaintext = b"\x12\x34\x56\x78\x90\x12\x34\x56\x12\x34\x56\x78\x90\x12\x34\x56";
        let cencrypt0 = COSEEncrypt0::new(plaintext, Cipher::aes_128_cfb128(), key);
    ...
    fn test_decrypt_invalid_alg_spec() {
        let key = b"\x00\x01\x02\x03\x04\x05\x06\x07\x08\x09\x0A\x0B\x0C\x0D\x0E\x0F";
        let plaintext = b"\x12\x34\x56\x78\x90\x12\x34\x56\x12\x34\x56\x78\x90\x12\x34\x56";
        let mut cencrypt0 = COSEEncrypt0::new(plaintext, Cipher::aes_128_gcm(), key).unwrap();
        let mut protected = HeaderMap::new();
        protected.insert(1.into(), CborValue::Text("invalid".to_string()));
    ...
    fn test_decrypt_unsupported_openssl_cipher() {
        ...
        let mut cencrypt0 = COSEEncrypt0::new(plaintext, Cipher::aes_128_gcm(), key).unwrap();
        let mut protected = HeaderMap::new();
        protected.insert(1.into(), CborValue::Integer(42));
        let protected_bytes = map_to_empty_or_serialized(&protected)
            .map_err(COSEError::SerializationError)
            .unwrap();
        cencrypt0.protected = ByteBuf::from(protected_bytes);
    ...
    fn test_decrypt_invalid_iv() {
        ...
        let mut cencrypt0 = COSEEncrypt0::new(plaintext, Cipher::aes_128_gcm(), key).unwrap();
        let mut unprotected = HeaderMap::new();
        unprotected.insert(5.into(), CborValue::Integer(42));
